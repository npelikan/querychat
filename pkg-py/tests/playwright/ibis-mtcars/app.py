...
def server(input, output, session):
    ...
    def use_google_models(system_prompt: str) -> chatlas.Chat:
        # Use Google's Gemini models with VCR integration
        chat = chatlas.ChatGoogle(
            model="gemini-2.5-flash-lite",
            system_prompt=system_prompt,
        )
        
        # Patch the chat method to use VCR
        original_chat_method = chat.chat
        
        def vcr_chat_method(prompt, *args, **kwargs):
            # Using a hardcoded name makes the test more deterministic and faster
            cassette_name = "ibis_mtcars_app_chat"
            
            # Create cassette directory
            cassette_dir = Path(os.path.dirname(os.path.abspath(__file__))) / "cassettes"
            os.makedirs(cassette_dir, exist_ok=True)
            
            # Create direct VCR instance with valid record_mode
            vcr_direct = vcr.VCR(
            ...
            # Use VCR for consistent API responses
            with vcr_direct.use_cassette(f"{cassette_name}.yaml"):
                result = original_chat_method(prompt, *args, **kwargs)
                return result
        
        # Replace just the chat method with VCR-wrapped version
        chat.chat = vcr_chat_method
        # Don't modify register_tool to avoid compatibility issues
