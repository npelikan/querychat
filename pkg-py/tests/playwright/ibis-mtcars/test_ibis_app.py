...
def test_ibis_app_loads(page: Page, local_app):
    ...
    # Wait for app to load (reduced timeout with VCR)
    ...
    assert page.locator("text=Average MPG >> nth=0").is_visible(), (
        "Average MPG box should be visible"
    )
    assert page.locator("text=Average Horsepower >> nth=0").is_visible(), (
        "Average HP box should be visible"
    )
    assert page.locator("text=Cars in Dataset >> nth=0").is_visible(), (
        "Cars count box should be visible"
    )
...
def test_ibis_app_query(page: Page, local_app):
    ...
    # With VCR, responses are faster, but we still need a little time for the UI to update
    ...
    table_selector = (
        ".rt-table, table, .table, #car_data, .data-grid, [role='grid'], [role='table']"
    )
    ...
    has_content = page.evaluate(
        f"() => {{ const el = document.querySelector('{escaped_selector}'); return el && el.children.length > 0; }}"
    )
    ...
    high_mpg_threshold = 20  # Define a named constant instead of magic number
    if mpg_values:
        assert max(mpg_values) > high_mpg_threshold, (
            "Should show cars with high MPG values"
        )
...
def test_ibis_app_plots(page: Page, local_app):
    ...
    assert page.locator("text=MPG vs Horsepower").is_visible(), (
        "MPG vs HP plot title should be visible"
    )

    # Check that the Cylinder Distribution plot is visible
    assert page.locator("text=Cylinder Distribution").is_visible(), (
        "Cylinder plot title should be visible"
    )
    ...
    assert page.locator("#mpg_hp_plot").is_visible(), (
        "MPG vs HP plot should still be visible after query"
    )
    assert page.locator("#cyl_plot").is_visible(), (
        "Cylinder plot should still be visible after query"
    )
...
def test_ibis_app_tab_navigation(page: Page, local_app):
    ...
    assert overview_tab.evaluate(
        "el => el.classList.contains('active') || el.getAttribute('aria-selected') === 'true' || window.getComputedStyle(el).fontWeight === 'bold'"
    ), "Overview tab should be active initially"

    # Verify the Overview tab content is visible
    assert page.locator("#avg_mpg").is_visible(), (
        "Average MPG box should be visible in Overview"
    )
    assert page.locator("#mpg_hp_plot").is_visible(), (
        "MPG vs HP plot should be visible in Overview"
    )
    ...
    assert car_details_tab.evaluate(
        "el => el.classList.contains('active') || el.getAttribute('aria-selected') === 'true' || window.getComputedStyle(el).fontWeight === 'bold'"
    ), "Car Details tab should be active after clicking"

    # Verify the Car Details content is visible
    table_selector = (
        ".rt-table, table, .table, #car_data, .data-grid, [role='grid'], [role='table']"
    )
    assert page.locator(table_selector).first.is_visible(), (
        "Car data table should be visible in Car Details tab"
    )
    ...
    assert overview_tab.evaluate(
        "el => el.classList.contains('active') || el.getAttribute('aria-selected') === 'true' || window.getComputedStyle(el).fontWeight === 'bold'"
    ), "Overview tab should be active after clicking back"
    assert page.locator("#mpg_hp_plot").is_visible(), (
        "MPG vs HP plot should be visible after going back to Overview"
    )
...
def test_ibis_app_plot_updates(page: Page, local_app):
    ...
    # Check that plots exist - we'll validate their visibility
    # We don't need to store the initial state since we're just checking visibility
    ...
    # Check that plots are still available after the update

    # The plots should still be visible
    assert page.locator("#mpg_hp_plot").is_visible(), (
        "MPG vs HP plot should still be visible"
    )
    assert page.locator("#cyl_plot").is_visible(), (
        "Cylinder plot should still be visible"
    )
    ...
    assert page.locator("#mpg_hp_plot").is_visible(), (
        "MPG vs HP plot should be visible after multiple queries"
    )
    assert page.locator("#cyl_plot").is_visible(), (
        "Cylinder plot should be visible after multiple queries"
    )
...
def test_ibis_app_sequential_queries(page: Page, local_app):
    ...
    assert page.locator(table_selector).first.is_visible(), (
        "Car data table should be visible"
    )
    # Use raw string and format manually to avoid f-string escaping issues
    js_code = (
        "() => { const el = document.querySelector('"
        + table_selector.replace("'", "\\'")
        + "'); return el && el.children.length > 0; }"
    )
    has_content = page.evaluate(js_code)
