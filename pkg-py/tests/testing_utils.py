    ...
    async def stream_async(self, prompt: str, echo: str = "none"):
        ...
        if "greeting" in prompt.lower() or "Include a few sample prompts" in prompt:
            response = self.responses.get(
                prompt,
                "👋 Welcome to the mock chat! Try asking: \n* Show me the data\n* Filter for high values",
            )
            return MockStreamResponse(response)
...
def create_mock_sql_chat(
    ...
    responses = {
        ...
        "Please give me a friendly greeting. Include a few sample prompts in a two-level bulleted list.": "👋 Welcome! Here are some questions you can ask:\n\n* About the data\n  * Show me the first 5 rows\n  * What columns are available?\n* Analysis\n  * Calculate summary statistics\n  * Are there any missing values?",
    }
...
def mock_chatlas_factory(mock_chat):
    """Create mock chatlas module with preconfigured Chat."""
